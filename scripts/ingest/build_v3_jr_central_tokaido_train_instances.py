...
OUTPUT_PATH = ROOT / "data" / "v3_tokyo_jr_central_tokaido_weekday_train_instances.json"
CACHE_DIR = ROOT / "data" / "v3_external" / "jr_central_tokaido"
...
def normalize_station_name(name: str) -> str:
    text = " ".join(html.unescape(str(name or "")).replace("\u3000", " ").split())
    text = re.sub(r"[（(].*?[）)]", "", text)
    text = re.sub(r"［.*?］", "", text)
    text = re.sub(r"\[.*?\]", "", text)
...
def station_name_variants(name: str) -> set[str]:
    ...
    variants = {text, text.replace("ケ", "ヶ"), text.replace("ヶ", "ケ")}
...
def load_station_seed() -> list[dict]:
    ...
    for feature in geojson["features"]:
        ...
        if props.get("N02_004") != "東海旅客鉄道":
            continue
        if props.get("N02_003") != "東海道線":
            continue
        ...
        out[name] = {
            "station_id": f"JR_CENTRAL_TOKAIDO_{name}",
            "name_ja": name,
            "operator": "東海旅客鉄道",
            "line_id": "東海道線",
            "lat": round(lat, 8),
...
def discover_navitime_nodes(station_seed: list[dict]) -> list[dict]:
    ...
    for station in station_seed:
        ...
        url = f"{AUTOCOMPLETE_URL}?word={quote(station['name_ja'])}"
        try:
        ...
        nodes.append({"station_name": station["name_ja"], "station_id": station["station_id"], "node_id": node_id})
    return nodes
...
def train_code_from_stop_url(stop_url: str) -> str:
    match = re.search(r"/diagram/stops/[^/]+/([^/?]+)/", stop_url)
    return match.group(1) if match else hashlib.sha1(stop_url.encode("utf-8")).hexdigest()[:12]
...
def parse_stop_page(stop_url: str, station_lookup: dict[str, dict]) -> dict | None:
    ...
    if title and "(" in title:
        line_name = title.split("(", 1)[0].replace("ＪＲ", "").replace("JR", "") or line_name
    ...
    for i, line in enumerate(lines):
        ...
        for nxt in lines[i + 1 : i + 6]:
            ...
            if not arrival:
                ...
                break
        if arrival:
            station_rows.append((station, arrival, departure or arrival))
    ...
    train_code = train_code_from_stop_url(stop_url)
    return {
        "service_instance_id": f"JR_CENTRAL_TOKAIDO_{train_code}_{SERVICE_DAY}",
        "train_number": train_code,
        "service_name": line_name or "東海道線",
        "headsign": headsign,
...
def dedupe_source_reports(source_reports: list[dict]) -> list[dict]:
    by_node_id: dict[str, dict] = {}
    no_node_reports: list[dict] = []
    for report in source_reports:
        node_id = report.get("node_id")
        if not node_id:
            no_node_reports.append(report)
            continue
        by_node_id[str(node_id)] = report
    return no_node_reports + list(by_node_id.values())


def write_output(station_seed: list[dict], source_reports: list[dict], train_instances: list[dict], node_seed: list[dict]) -> None:
    clean_source_reports = dedupe_source_reports(source_reports)
    payload = {
        "id": "v3_tokyo_jr_central_tokaido_weekday_train_instances_v0_1",
        "label": "v3 Tokyo JR Central Tokaido Line weekday train instances",
        "version": "0.1.0",
...
def main() -> int:
    ...
    force_stations = {
        name.strip()
        for name in os.environ.get("FORCE_STATIONS", "").split(",")
        if name.strip()
    }
    process_node_seed = [
    ...
    seen_stop_urls = {
        train.get("source_url")
        for train in train_instances
        if train.get("source_url")
    }
    seen_train_station_pairs = {
        (train.get("train_number"), stop.get("station_id"))
        for train in train_instances
        for stop in train.get("stop_times", [])
        if train.get("train_number") and stop.get("station_id")
    }
    ...
    for idx, node in enumerate(process_node_seed, start=1):
        ...
        if node_id in completed_nodes and node["station_name"] not in force_stations:
            continue
        try:
            ...
            print(f"[jr-central-tokaido] node error {idx}/{len(process_node_seed)} {node_id} -> {type(exc).__name__}: {exc}")
            continue

        added = 0
        for timetable_url in timetable_urls:
            if "東海道" not in fetch_text(timetable_url) and "tokaido" not in timetable_url.lower():
                continue
            try:
            ...
            for stop_url in stop_urls:
                if stop_url in seen_stop_urls:
                    continue
                train_code = train_code_from_stop_url(stop_url)
                current_station_pair = (train_code, node["station_id"])
                if current_station_pair in seen_train_station_pairs:
                    continue
                try:
                ...
                for stop in parsed.get("stop_times", []):
                    seen_train_station_pairs.add((train_code, stop.get("station_id")))
                if action == "added":
                    added += 1
                if len(train_instances) >= next_train_checkpoint:
                    write_output(station_seed, source_reports, train_instances, node_seed)
                    print(f"[jr-central-tokaido] train checkpoint {len(train_instances)} after node {idx}/{len(process_node_seed)}")
        ...
        write_output(station_seed, source_reports, train_instances, node_seed)
        print(f"[jr-central-tokaido] checkpoint {idx}/{len(process_node_seed)}: {len(source_reports)} nodes, {len(train_instances)} trains")

    write_output(station_seed, source_reports, train_instances, node_seed)
    print(f"[jr-central-tokaido] wrote {len(train_instances)} train instances -> {OUTPUT_PATH}")
    return 0
