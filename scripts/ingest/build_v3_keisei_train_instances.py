...
OUTPUT_PATH = ROOT / "data" / "v3_tokyo_keisei_weekday_train_instances.json"
CACHE_DIR = ROOT / "data" / "v3_external" / "keisei"
TIMEOUT = 30
MAX_FETCH_RETRIES = 5
...
def discover_detail_urls(station_page_url: str) -> list[str]:
    ...
    for tx, sf, date, minute, dw in re.findall(
        ...
        url = (
            f"{BASE_URL}/search/timetable/onetraintimetable/"
...
def parse_detail_page(detail_url: str, station_lookup: dict[str, dict]) -> dict | None:
    ...
    service_instance_id = f"KEISEI_{train_number}_{SERVICE_DAY}_{departure_seed}"
    ...
    for station_index, station_page in enumerate(station_pages, start=1):
        ...
        for detail_url in detail_urls:
            ...
            if len(train_instances) >= next_train_checkpoint:
                ...
                print(
                    f"[keisei] train checkpoint {len(train_instances)} "
        ...
        if station_index % CHECKPOINT_EVERY == 0:
            write_output(station_seed, source_reports, train_instances)
            print(
                f"[keisei] checkpoint {station_index}/{len(station_pages)}: "
                f"{len(source_reports)} source pages, {len(train_instances)} trains"
            )

    write_output(station_seed, source_reports, train_instances)
    print(f"[keisei] wrote {len(train_instances)} train instances -> {OUTPUT_PATH}")
    return 0
