...
def extract_train_links(html: str, page_url: str) -> list[str]:
    links = re.findall(r'href="(/train-timetable/\d+\?date=\d+)"', html)
    deduped: list[str] = []
    seen: set[str] = set()
    for link in links:
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Build train instances from a JR West station timetable page.")
    ...
    train_links = extract_train_links(timetable_html, args.timetable_url)
    ...
    for train_url in train_links:
        ...
        if len(kept_urls) % 25 == 0:
            print(f"Fetched {len(kept_urls)} JR West train pages...", flush=True)
