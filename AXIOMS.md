...
- At the end of every work turn, tell Scorp the concrete next step. If the work is fully complete, still state the best next optional direction or the next maintenance check.
...
- Player-facing route choices must classify the immediate boarding segment by the current physical large line only, except for reviewed public operating corridors whose name is the passenger-facing boarding identity on that core segment. Remote through-running labels such as `横須賀線`, `総武快速線`, `京葉線`, or similar destination-side service identities may appear in selected-train labels when the train is chosen, but must not replace `内房線`, `外房線`, `成田線`, or any other current physical boarding line in the route-choice list.
- `湘南新宿ライン` is a reviewed JR East public operating corridor, not an independent physical line. It may be a player-facing route choice only on its core `大宮 - 新宿 - 横浜 - 大船` corridor; outside that core, branch stations continue to show the current physical line such as `宇都宮線`/`東北本線`, `高崎線`, `東海道線`, or `横須賀線`. Selected-train highlighting must still follow the train's real physical path.
...
- Coupled split/join trains (`併結運転` / `分割・併合`, similar to `多層建て列車`) are a separate model from ordinary through-running. When reviewed coupled portions represent one physical passenger train at the boarding point, show one umbrella train name joined with the Japanese middle dot, such as `関空快速・紀州路快速`. If the train is still heading toward a future split, selecting the umbrella must first ask which portion/direction the player will ride, then show alighting stops for that portion. In the reverse direction, before the join point the portions are still separate trains; after the join point they can be shown as one physical train and should go directly to alighting stops because there is no future branch choice. During the shared coupled segment, all portions in the same coupled physical group count as `same_train` for capture.
...
- In a selected train's alighting list, transfer chips must not list the same train's own through-running physical route as a transfer at ordinary intermediate stops. Only true route-boundary stops may expose the adjacent through route as a transfer opportunity.
...
- Newly collected gameplay transportation data is not complete merely because it exists in a source/cache/audit file. For V5 public-transport modes, collected data should be promoted into the playable runtime bundle and planner tiles as part of the same work whenever possible. If a collected source cannot be made playable yet, record the blocker explicitly, such as missing coordinates, incomplete stopTimes, unresolved calendars, duplicate overlap, or image/OCR-only source format.
- For ferry/ship port connectors, never attach rail or bus access by port name alone. Generic or reused names such as `大島港` must first be resolved by operator, route context, region, and precise coordinate; otherwise the connector can silently attach a real bus stop to the wrong island or city.
...
- When a timetable source labels a train by a branch line but the first or last public stop sits on an adjacent trunk line, the selected-train path must include the real non-stopping boundary segment through the junction station. Examples include `石北線` services reaching `旭川` via `新旭川`, `徳島線` services reaching `徳島` via `佐古`, and `芸備線` services reaching `新見` via `備中神代`. Add reviewed path hints for the physical junction instead of drawing a direct station-to-station line.
