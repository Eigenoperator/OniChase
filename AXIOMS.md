...
- Coupled split/join trains (`併結運転` / `分割・併合`, similar to `多層建て列車`) are a separate model from ordinary through-running. When a train is moving from the coupled shared segment toward the split direction, the first train-selection row should show one umbrella train name joined with the Japanese middle dot, such as `はやぶさ・こまち`; after selecting it, the player must choose the concrete portion before alighting stations are shown. When moving from the uncoupled side toward the coupled shared segment, show the portions as normal separate trains. During the shared coupled segment, all portions in the same coupled physical group count as `same_train` for capture.
- `東北・北海道新幹線` and `秋田新幹線` coupled Shinkansen services share one player-facing route-choice corridor on the Tokyo-Morioka shared segment. Show that corridor as `東北・北海道・秋田新幹線`; after the movement leaves the shared segment at Morioka, keep the branch-side route choices separate by their real physical path.
