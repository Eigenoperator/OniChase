...
- Coupled split/join trains (`併結運転` / `分割・併合`, similar to `多層建て列車`) are a separate model from ordinary through-running. For non-Shinkansen coupled trains, when a train is moving from the coupled shared segment toward the split direction, the first train-selection row should show one umbrella train name joined with the Japanese middle dot, such as `関空快速・紀州路快速`; after selecting it, the player must choose the concrete portion before alighting stations are shown. When moving from the uncoupled side toward the coupled shared segment, show the portions as normal separate trains. During the shared coupled segment, all portions in the same coupled physical group count as `same_train` for capture.
