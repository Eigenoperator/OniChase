...
- 已为本地客户端加入 `Settings` 按钮和首个设置项：现在可以在设置窗里调整整个 UI 的字体大小，默认首屏字号也已显著调大，并提高了字体滑条上限。
...
- 正在把本地客户端从“可直接跑局”继续推进到“更像正式玩法工作台”的阶段，重点是时间流、回放、地图联动。
...
3. 继续把本地客户端回放区做得更强，比如支持更明显的当前事件高亮和地图联动。
