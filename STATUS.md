...
- Current v4 structured train collection has `125331` weekday train instances across `22` source collections and `179` operators after the global station-match-distance quality gate.
...
- v4 gameplay bundle now exports `114404` playable compact trips, `605` service routes/patterns, and skips `309` short gameplay trips after compact dedupe.
...
- JR West duplicate collection merging now also preserves the `関西空港線` variant for duplicate airport-bound trains instead of letting same-train-number `阪和線`/`大阪環状線` variants mask it. Current data now keeps all `25` weekday `日根野` -> `関西空港` trains under `関西空港線`; browser probes show `関西空港線` departures from 日根野 from `05:34`, with `はるか` still exposed as a separate named limited-express choice.
