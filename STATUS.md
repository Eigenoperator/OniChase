...
- v4 route-choice QA now globally scans all `9052` station groups for player-facing route choices that do not match the current boarding stop -> next stop segment or escape their allowed virtual-corridor station set; latest scan reports `0` anomalies after enforcing Tokyo-station directionality (`東海道線` south, `上野東京ライン` north), tightening JR East virtual corridors, and fixing りんかい線直通 display.
