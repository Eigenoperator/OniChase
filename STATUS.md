...
- v4 route-choice QA now globally scans all `9052` station groups for segment mismatches, virtual-corridor leaks, generic `路線`, remote Yokohama through routes/labels, selected-train label violations, JR East northern-trunk scope, major-station Shinkansen visibility, transfer-equivalent station group route completeness, and post-train-selection alighting availability; latest scans report `1675021` rows, `1675374` choices/labels, `74` equivalent clusters including 名古屋/名鉄名古屋/近鉄名古屋, `493` pending-trip checks, and `0` anomalies/missing routes.
- v4 branded-prefix transfer audit now scans `65` JR/private candidate interchanges: `3` reviewed direct, `1` reviewed non-direct (`蒲田`/`京急蒲田`), `49` active unreviewed direct, and `12` nearby walking candidates needing review.
