...
- v4 long-distance playability audit now combines `30` fixed staged routes with seeded random route-chain planning (`20` random cases by default), then audits every reached origin/transfer/final station surface; latest run reports `50/50` playable, `137` waypoint station surfaces checked, and `0` anomalies.
