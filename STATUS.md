...
- v4 route-choice QA now globally scans all `9052` station groups for segment mismatches, virtual-corridor leaks, generic `路線`, remote Yokohama through routes/labels, selected-train label violations, and JR East northern-trunk scope (`東北本線` main, `常磐線` from 上野, `高崎線` from 大宮, `上野東京ライン` nickname only between 東京 and 上野); latest scan reports `1675021` rows, `1675374` choices/labels, and `0` anomalies.
