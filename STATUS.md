...
- 已完成一轮 `v1` 开发期自测：用真实山手线时刻表跑了 10 组代表性对局，覆盖开局同站抓捕、hunter 追到静止 runner、故意追错站抓不到、同车抓捕等情况，当前引擎层结果都符合预期。
- 已绘制第一张 `v2` 新干线全图战略草图，放在 `visuals/shinkansen_v2_map_draft.svg`，用于检查从单线山手线扩到多线全国新干线时的追逃空间与关键枢纽布局。
## In Progress
...
## Next
1. 检查这张 `v2` 新干线草图的线路范围、关键枢纽和视觉方向是否符合我们想要的战略空间。
2. 继续做 `v1` 的 hunter / runner 开发期实机试玩，重点看 GUI 层是否还有“能判但不好操作”的问题。
3. 根据后续实机试玩结果和 `v2` 草图反馈继续调整客户端与地图方向。
