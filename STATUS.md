...
- v4 route-choice QA now globally scans all `9052` station groups for player-facing route choices that do not match the current boarding stop -> next stop segment, escape their allowed virtual-corridor station set, or violate selected-train label rules; latest scan reports `0` anomalies after enforcing the `東海道線`-`東京`-`上野東京ライン`-`上野`-branch chain and train labels with no raw `x号線`.
