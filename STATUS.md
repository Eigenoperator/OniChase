...
- 已继续修正 hunter 测试中的交互与裁定可见性：站点不再容易被玩家标记盖住，双端 `LIVE` 里一旦发生 `same_node / same_train` 抓捕就会立刻结束并在地图上标出捕获点，同时会显示更清楚的抓捕说明卡。
## In Progress
...
## Next
1. 开始实际试玩 `v1` 的 hunter mode，重点检查 `07:00 -> 08:00` 时间窗下的抓捕判定是否和地图上的 game-end 标记一致。
2. 根据 `v1` 的 hunter 实测结果继续调整客户端里的信息提示、地图表达、结束反馈和结果说明。
