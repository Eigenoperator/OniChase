...
- v3 regression now includes one-command heavy battle validation, P0 axiom unittests, canonical replay-record validation, and v3 page source-of-truth sync checks.

## In Progress
- Continue hardening `v2` online playtest details: room state, ready/planning/live sync, single-player and multiplayer parity.
- Stabilize `v3` MapLibre/reporting UX behind automated checks: dense Tokyo click behavior, player marker clarity, battle timelines, replay UI/share flow, and map/planner readability.
...
## Next
1. Run the v3 axiom/replay/page suites before changing bundle, through-running, filtering, selected-train highlight, capture, replay, or build-source logic; next product step is replay UI/share flow, then easy single-player AI.
