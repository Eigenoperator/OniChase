...
- Current v4 structured train collection has `126002` weekday train instances across `22` source collections and `178` operators after the global station-match-distance quality gate.
...
- v4 gameplay bundle now exports `115044` playable compact trips, `600` service routes/patterns, and skips `309` short gameplay trips after compact dedupe.
...
- JR West duplicate collection merging now preserves the line-specific context for 湖西線 train variants instead of letting same-train-number 東海道線/山陽線/北陸線 variants mask them. 湖西線 coverage is back to `190` JR West official trains in the current collection; browser station probes show ordinary 湖西線 departures at 近江今津, 堅田, 山科, and 近江舞子.
...
2. Continue the remaining `24/200` blind-route failures by separating true timetable gaps from wrong hand-authored waypoints; priority suspects are Shibayama Railway, Shimabara Railway, Matsuura Railway, Toyama Chitetsu rail, and several same-name/route-title disambiguation cases.
