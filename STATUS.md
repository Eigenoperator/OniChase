...
- v4 route-choice QA now globally scans all `9052` station groups for player-facing route choices that do not match the current boarding stop -> next stop segment, escape allowed virtual-corridor station sets, show generic `路線`, expose remote Yokohama through routes/labels, or violate selected-train label rules; latest scan reports `1675021` rows, `1675374` choices/labels, and `0` anomalies.
