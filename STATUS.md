...
- v4 route-choice QA now globally scans all `9052` station groups for player-facing route choices that do not match the current boarding stop -> next stop segment, escape their allowed virtual-corridor station set, show generic `路線`, or violate selected-train label rules; latest scan reports `0` anomalies after enforcing the through-line chain, direction-side ordinary labels, and no raw `x号線`.
