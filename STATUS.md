...
- Public `v3` now reuses the `v2` UI code path with `1679` station groups, `2048` physical stations, `108` service routes, `4226` track centerlines, and all `40456` v2-compatible trip instances.
...
- Refined v3 planning rows: terminal-only trains and duplicate boardable services are filtered, direct-through aliases are hidden from line choices/transfers, compact two-row line-symbol assets are used, Keikyu airport trains reach Haneda terminals, and JR East `成田線` geometry now covers the `我孫子 -> 湖北 -> 新木 -> 成田` branch.
