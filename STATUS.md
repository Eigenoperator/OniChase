...
- v3 replay is now first-class in the MapLibre UI: visible Result/Replay panel, canonical `v3.replay.1` records, copyable share links/JSON, hash loading, and replay UI regression coverage.

## In Progress
- Continue hardening `v2` online playtest details: room state, ready/planning/live sync, single-player and multiplayer parity.
- Stabilize `v3` MapLibre/reporting UX behind automated checks: dense Tokyo click behavior, player marker clarity, richer replay playback, easy single-player AI, and map/planner readability.
...
## Next
1. Run the v3 axiom/replay/page suites before changing bundle, through-running, filtering, selected-train highlight, capture, replay, or build-source logic; next product step is richer replay playback, then easy single-player AI.
