...
- 已为本地端和网页端都加入 `Start Game` 按钮：当前试玩版允许在 `PLANNING` 中手动提前开局，同时在界面中明确说明正式对局应当由双方同意后再开始。
...
## Next
1. 开始实际试玩 `v1` 的 hunter mode，重点检查 `07:00 -> 08:00` 时间窗下“站点可见 / 站间粗位置 / live 全隐藏”这套信息闭环是否好懂。
