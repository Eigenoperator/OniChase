...
- v4 selected-train path highlighting now checks whether a full trace slice actually covers intermediate stops, falls back to scored adjacent-stop physical route segments when a trace is too coarse, and has a reviewed via-station hint for 大阪環状線 大阪↔天王寺 airport-bound services. The regression audit now verifies the first `40` 大阪→関西空港 cases, including `はるか`, highlight the west side via 西九条/弁天町/大正 and not the 京橋/鶴橋 side.
