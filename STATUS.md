...
- v4 long-distance playability audit now runs `100` fixed-class cases (`30` hand-authored plus `70` novelty-biased coverage cases) and `100` seeded random route-chain cases by default, then audits every reached origin/transfer/final station surface; latest run reports `200/200` playable, `682` waypoint station surfaces, `495` unique waypoint stations, `165` unique leg routes, and `0` anomalies.
