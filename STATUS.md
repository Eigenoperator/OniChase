...
- 已把 `PLAN BOARD` 从右侧滚动内容中分离成固定区域，修正右侧动作区刷新后会把手动滚动位置弹回顶部的问题，并减少右侧 `PLAN BOARD` 与 `STEP 1 / STEP 2` 因整块重复重建导致的闪烁；玩家在车上时现在也会沿站间线路连续显示，不再只贴起终点站。
...
3. 继续把本地客户端回放区做得更强，比如支持更明显的当前事件高亮、播放控制和事件步进。
